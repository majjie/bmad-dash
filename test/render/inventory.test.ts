...
// ---------------------------------------------------------------------------
// AD-17: the identity crosses the projection
// ---------------------------------------------------------------------------
...
test('the identity crosses the projection: equal for two unchanged passes, and it moves when the project does', async (t) => {
  ...
  assert.match(first.snapshotId, /^[0-9a-f]{16}$/, 'the identity is present and opaque');
  ...
  assert.match(FULL_INVENTORY_VIEW.snapshotId, /^[0-9a-f]{16}$/);
});

test('an empty project still derives an identity, and it is stable', async (t) => {
  // The I/O matrix's empty-project row, which needs its own test: the row
  // above asserts `EMPTY_INVENTORY.snapshotId` is hex, but that fixture
  // carries a *hardcoded* id, so it shows the field exists rather than that
  // the projection derives one. A markers-only project is the case where every
  // digest input is empty or zero — no rows, no groups but the story family,
  // `artifactCount` 0 — which is exactly where a derivation could plausibly
  ...
  // empty case is not collapsing to a constant the derivation ignores its
  // inputs for.
...
/**
 * The inputs iteration 1 of this story digested, and nothing else.
 *
 * Every pair in the matrix below is *equal* under this and renders
 * differently, which is what makes the matrix load-bearing: an implementation
...
function iterationOneInputs(view: InventoryView): string {
  const rows = view.groups.flatMap((group) =>
    group.rows.map((row) => [row.path, row.identity.outcome, row.readability.state].join('|')),
  );
...
test('a rendering difference is an identity difference, for every field the view carries', async () => {
  ...
  const pairs: readonly (readonly [string, InventoryView, InventoryView])[] = [
    ...
    [
      'a different readability stage under one readability state',
      oneGroup([rowAt({ readability: { state: 'unreadable', stage: 'decode' } })]),
      oneGroup([rowAt({ readability: { state: 'unreadable', stage: 'read' } })]),
    ],
    [
      "an ambiguous verdict's second reading",
      oneGroup([
        rowAt({
          identity: {
            outcome: 'ambiguous',
    ...
    [
      'a changed run fact',
      oneGroup([rowAt({ runFacts: [{ measured: true, reuse: 'accidental', dateSignal: 'present' }] })]),
      oneGroup([rowAt({ runFacts: [{ measured: true, reuse: 'deliberate', dateSignal: 'present' }] })]),
    ],
    [
        ...
        groups: [
          {
            family: 'story',
            rows: [CERTAIN_ROW],
            notes: [{ kind: 'story-location', report: { kind: 'at', path: '_bmad-output/sprint-status.yaml' } }],
          },
        ],
      }),
    ],
    ...
    [
      ...
      oneGroup([rowAt({ interpretation: 'interpreted' })]),
      oneGroup([rowAt({ interpretation: 'present-but-uninterpreted' })]),
    ],
  ];
...
test('a project root the pass could not read is not reported as a finished scan', async (t) => {
  ...
  await whileDenied(root, async () => {
    ...
    assert.match(view.snapshotId, /^[0-9a-f]{16}$/, 'the identity still exists for an unreadable root');
