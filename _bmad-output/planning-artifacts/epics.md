...
**Satisfies:** AD-3 (the snapshot half), AD-17 (the identity half — see below) · no new FRs; this story is architecture-driven, and its absence is what would make later stories quietly wrong rather than visibly incomplete

**Scope corrected 2026-09-03**, after investigation, from a first draft of this entry that over-claimed. **AD-17 is split across epics by this plan itself**: Story 3.6 also satisfies AD-17, alongside FR-37 and FR-47, and it owns the *replacement* half — refresh not mutating in place, the previous snapshot staying live when a scan fails. Refresh does not exist in Epic 2 at all (FR-37 and FR-47 are Epic 3), so a mid-read refresh is neither buildable nor testable here and that check belongs to 3.6. The **currency probe** belongs to AD-11, which is Story 2.2's. And AD-3's eager index names *timestamps*, which nothing in the tool handles yet — normalization is FR-67, Story 3.10 — so this story extracts what the pass already extracts and adds no time handling.

**Done when:** one scan produces one immutable snapshot carrying an identity; the snapshot does bounded work per artifact and builds no rendering representation; every response that carries project content records the snapshot identity it was produced from, and the responses produced before a snapshot exists — the `Host` refusal, the method refusal, an unknown path — are exempt by stated reason rather than by omission; and the identity is derived from what was scanned, not from a clock or a counter, so it is stable while the project is unchanged. The check that matters: two loads of an unchanged project carry the same identity and are byte-identical, and a project that changed between loads carries a different one — which is the property Story 2.1a's rendering cache is keyed on.
