...
- [x] `scripts/run-tests.mjs` -- the discovery guard. `node --test` exits 0 when a pattern matches nothing, and a single-level glob silently omits subdirectories, so a green run is not evidence the suite ran. Runs the recursive pattern, parses the reported total, fails below a floor, and accepts env overrides so the guard can be tested by the suite it guards *(Ticked but the path is wrong: this file is `run-tests.ts`, not `.mjs` — open finding #112.)*
